...
public class AttackMessageBox : BaseMessageBox
{
    ...
    public override void DisplayMessage(BaseMessage entry)
    {
        ...
        tmpAttackUserText.text = $"<color={attackUserColor}>{character.characterName}</color>";
        tmpAttackTargetText.text = $"<color={attackTargetColor}>{character.currentTarget.characterName}</color>";
