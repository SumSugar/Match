...
public class CameraEffectsManager : MonoBehaviour
{
    ...
    public void OnCharacterDeath(object obj)
    {
        ...
        TriggerZoomToCharacter(character.transform , 2.5f, 1.0f);
    ...
    public void TriggerZoomToCharacter(Transform characterTransform, float zoomLevel, float zoomDuration)
    {
        ...
        // 拉近到角色位置
        Vector3 zoomPosition = new Vector3(characterTransform.position.x, characterTransform.position.y+2f, originalPosition.z);
        transform.DOMove(zoomPosition, zoomDuration).SetId("ZoomMove").SetUpdate(true); // 使用 SetUpdate(true) 保证不受时缓影响

        // 改变摄像机的缩放
        mainCamera.DOOrthoSize(zoomLevel, zoomDuration).SetId("ZoomMove").SetUpdate(true).OnComplete(() =>
        {
            // 缩放回默认的初始值
            mainCamera.DOOrthoSize(originalZoom, zoomDuration).SetId("ZoomMove").SetUpdate(true);
            transform.DOMove(originalPosition, zoomDuration).SetId("ZoomMove").SetUpdate(true); // 回到初始位置
        });
    }
